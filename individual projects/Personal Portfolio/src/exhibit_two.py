...
folder_path = r"C:\Users\strat\JS_CP2\individual projects\Simple Grade Book\files"
file_path = os.path.join(folder_path, "main.py")

if folder_path not in sys.path:
    sys.path.append(folder_path)

spec = importlib.util.spec_from_file_location("sgb_main", file_path)
sgb_module = importlib.util.module_from_spec(spec)
sys.modules["sgb_main"] = sgb_module
...
def display():
    ...
    text = styles.OutputText(text_box.frame, text_color=from_rgb((0,0,0)), text_size=60, justify="left", text="Simple Grade Book\n\nThis was the project I finally cracked down on formatting.\nMy code from this project on finally really starting looking\nconsistent and from there are on started preforming at my best.")
    text.show()
